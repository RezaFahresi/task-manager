<x-app-layout :title="'Notification Center - Task Manager'">
    <div class="container-fluid px-3 px-md-4 py-4 max-w-6xl mx-auto">
        ...
        <div id="nc-container">
            <div id="nc-card" class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white mb-4 {{ $notifications->isEmpty() ? 'd-none' : '' }}">
                                ...
                                <div class="min-w-0">
                                    <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                                        ...
                                        @elseif ($type === 'reminder_10m')
                                            <span class="badge bg-danger text-white rounded-pill px-2 py-0.5 fw-bold d-inline-flex align-items-center gap-1 shadow-sm" style="font-size: 0.72rem;">
                                    ...
                                    <div class="d-flex align-items-center gap-3 text-secondary" style="font-size: 0.75rem;">
                                        ...
                                        @if (!empty($data['due_date']))
                                            ...
                                            <span class="d-inline-flex align-items-center gap-1">
                                                ...
                                                <span>Deadline: {{ \Carbon\Carbon::parse($data['due_date'])->format('d M Y' . (str_contains($data['due_date'], ':') ? ' H:i' : '')) }}</span>
                                            </span>
                                        @endif
                                    </div>
                                </div>
            ...
            @if ($notifications->hasPages())
                <!-- Pagination -->
                <div id="nc-pagination" class="mt-4">
                    {{ $notifications->links() }}
                </div>
            @endif

            <!-- Empty State -->
            <div id="nc-empty-card" class="card border-0 shadow-sm rounded-4 bg-white p-5 text-center {{ $notifications->isNotEmpty() ? 'd-none' : '' }}">
                <div class="py-4">
